	...
	public partial class MMPTests
	{
		...
		public void BuildUnifiedMobile_Program_SmokeTest ()
		{
			RunMSBuildTest (tmpDir => {
				string projectPath = TI.GenerateAppProject (new TI.UnifiedTestConfig (tmpDir) { ProjectName = "UnifiedExample.csproj" });
		...
		{
			RunMSBuildTest (tmpDir => {
				string projectPath = TI.GenerateAppProject (new TI.UnifiedTestConfig (tmpDir) { ProjectName = "XM45Example.csproj" });
				TI.BuildProject (projectPath, isUnified : true);
			});
		}
		...
		void TestBCLCore (string tmpDir, string projectName)
		{
			...
			string projectPath = TI.GenerateAppProject (new TI.UnifiedTestConfig (tmpDir) { ProjectName = projectName, References = reference, TestCode = testCode });
			TI.BuildProject (projectPath, isUnified : true);
		}
		...
		{
			RunMSBuildTest (tmpDir => {
				string projectPath = TI.GenerateAppProject (new TI.UnifiedTestConfig (tmpDir) { FSharp = true, ProjectName = "FSharpUnifiedExample.fsproj" });
				TI.BuildProject (projectPath, isUnified : true);
			});
		}
		...
		{
			RunMSBuildTest (tmpDir => {
				string projectPath = TI.GenerateAppProject (new TI.UnifiedTestConfig (tmpDir) { FSharp = true, ProjectName = "FSharpXM45Example.fsproj" });
				TI.BuildProject (projectPath, isUnified : true);
			});
		}
		...
		public void BuildUnifiedProject_WithJustNativeRefNoLinkWith_Builds()
		{
			RunMSBuildTest (tmpDir => {
				...
				string projectPath = TI.GenerateAppProject (new TI.UnifiedTestConfig (tmpDir) { ProjectName = "UnifiedExample.csproj", ItemGroup = itemGroup });
		...
		public void BuildingSameProject_TwoTimes_ShallNotInvokeMMPTwoTimes ()
		{
			RunMSBuildTest (tmpDir =>
			{
				foreach (var project in new string[] { "UnifiedExample.csproj", "XM45Example.csproj" })
				{
					...
					string projectPath = TI.GenerateAppProject (config);
					string buildOutput = TI.BuildProject (projectPath, isUnified: true);
